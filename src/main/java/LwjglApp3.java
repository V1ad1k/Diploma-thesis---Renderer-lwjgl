...
public class LwjglApp3 implements Runnable {
    ...
    public LwjglApp3(String arg, float x1, float y1, float z1, String nameStore, String videoName) {
        pathObj = arg;
        x = x1;
        y = y1;
        z = z1;
    ...
    public void saveImages(int times, boolean flag){
        if (times < 63){
...
//            z1 -= 0.24f;
            x1 -= 1.0;
            z1 -= 0.22f;
            scene = new Scene(new Vector3f(-x1, 3, -z1), new Vector3f(0, 0, 0), pathObj);
        }
        else {
            x1 += 1.0;
            z1 += 0.15f;
...
//        scene.dynamicRender2(i);
        if (new File(String.format("./data/frame%d.obj", i +1)).exists()) {
    ...
    public void run() {
        try {
            ...
            while(new File(String.format(path + "./storage/img%d.png", i)).exists()) {
    ...
    public static void main(String[] args) {
        ...
        new LwjglApp3(params.get("obj").get(0),  // obj file and automatically mtl
                ...
                params.get("video").get(0)).run(); // observer
    }
}
